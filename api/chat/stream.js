...
function cleanReasoningArtifacts(content) {
  ...
  // The model's reasoning is plain unformatted text. The actual answer
  // uses markdown: bold headings, tables, numbered lists with bold, or
  // starts a coherent long paragraph.
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (
      /^\*\*[A-Z]/.test(line) ||           // Starts with **Bold heading
      /^#{1,6}\s/.test(line) ||             // Markdown heading
      /^\|[^|]+\|[^|]+\|/.test(line) ||    // Table row with 2+ columns
      /^\d+\.\s+\*\*/.test(line) ||         // Numbered list with bold
      /^- \*\*/.test(line)                   // Bullet list with bold
    ) {
      answerStart = i;
      break;
    }

    // Check if line contains **bold** mid-line (e.g. 'Now produce final answer:**YC-backed...')
    ...
    // Long sentence starting with capital = likely the answer (not a reasoning line)
  ...
  // Step 5: Clean up excess whitespace
